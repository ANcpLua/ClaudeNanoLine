...
OAuth トークンに Usage API が要求する `user:profile` スコープがありません。
このスコープを要求しない OAuth フロー（例: Claude Code 管理のトークン）で典型的に
発生し、Claude Desktop のトークンでは発生しません。利用率 4 項目は `Forbidden`
と表示され、クライアント側の回避策では復旧できません。`/login` の再実行が有効
かどうかは Claude Code リリースで使われている OAuth クライアントに依存します。
ログには `error:forbidden http_status=403 (token scope insufficient)` が記録されます。
